    ...
    public function lastStep(Request $request)
{
    ...
    try {
        ...
        // 👉 STEP 1: Write ENV (safe, without killing session)
        $this->changeEnv([
            'APP_NAME'      => session('env.APP_NAME'),
            'APP_ENV'       => session('env.APP_ENV'),
            'APP_KEY'       => session('env.APP_KEY'),  // Already generated earlier
            'APP_DEBUG'     => session('env.APP_DEBUG'),
            'APP_URL'       => session('env.APP_URL'),
            'LOG_CHANNEL'   => session('env.LOG_CHANNEL'),
        ...
        // 👉 IMPORTANT: Don't clear config before using DB - it kills session!
        Artisan::call('config:clear');

        // 👉 STEP 2: Run migrations fresh + seed
        Artisan::call('migrate:fresh', [
            '--force' => true,
            '--seed'  => true,
        ]);

        // 👉 STEP 3: Passport migrations
        Artisan::call('migrate', [
            '--path' => 'vendor/laravel/passport/database/migrations',
            '--force' => true,
        ]);

        // 👉 STEP 4: Generate passport clients & keys
        Artisan::call('passport:install', [
            '--force' => true,
        ]);
        ...
        // 👉 STEP 6: Re-cache config (only at the end)
        Artisan::call('config:cache');
        ...
        // Return clean error message on screen
